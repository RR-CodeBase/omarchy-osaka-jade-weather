...
//
// The JSON file at statePath is the single source of truth. `osaka-weather`
...
Item {
  ...
  readonly property string statePath: Quickshell.env("HOME") + "/.local/state/omarchy/weather-fx.json"
  ...
  // drawn for. Defaults are that city; `osaka-weather sky` retunes them.
  //
  ...
  IpcHandler {
    ...
    // Nudge the watcher instead of waiting on it. `osaka-weather` has already
